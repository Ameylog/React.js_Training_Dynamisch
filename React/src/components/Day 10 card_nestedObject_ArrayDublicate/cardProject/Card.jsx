...
function Card({ arrObj }) {

    const [originalData, setOriginalData] = useState(arrObj)
    ...
    return (
        ...
        <div className='mt-4  m-3  py-4 px-4'>
            <div className="row row-cols-1 row-cols-md-2 row-cols-lg-3 g-3">
                    ...
                    originalData?.map((val, index) => {
                        ...
                        return <div className="card  mt-3 shadow-sm ms-3"
                            ...
                            <div className="card-body">
                                ...
                                {(max === min) ? "" : ((val.salary === min) ? <p className='bg-warning text-white w-100'>Lowest Salary</p> : "")}
                                {(max === min) ? "" : ((val.salary === max) ? <p className='bg-success text-white w-100'>Highest Salary</p> : "")}

                                {(val.age < 35) ? <p className='bg-danger text-white'>Applicable for job</p> : ""}
                ...
                {(originalData?.length === 0) ? <img src={NODataImg} alt='No Data Found'/> : ""}
            </div>
